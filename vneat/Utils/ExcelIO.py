...
class CSVSheet(object):
    ...
    class Row(TransformedDict):
        ...
        def __repr__(self):
            return 'CSVRow( ' + TransformedDict.__repr__(self) + ' )'

        def __str__(self):
            return 'CSVRow( ' + TransformedDict.__str__(self) + ' )'
    ...
    def __init__(self, filename, sheet_index=0, header_row=0):
        ...
        with open(filename,'r') as f:
            ...
            # Read it and extract headers and the columns where they are
            self._header2col = CSVSheet.Row()
            for col in range(len(headers)):
                if isinstance(headers[col],str):
                    self._header2col[headers[col]] = col
                else:
                    self._header2col[str(headers[col])] = col
    ...
    def get_rows(self, start=None, end=None, fieldstype=str):
        ...
        with open(self._filename, 'r') as f:
            ...
            if start is None:
                ...
                start == 0
            if end is None or end > len(reader):
                end = len(reader)
            ...
            for i in range(start, end):
                ...
                for field, col in yes:
                    # Valid elements are in
                    try:
                        r[field] = fieldstype[field](rinfo[field])
                    except:
                        r[field] = None
