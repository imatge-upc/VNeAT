...
nelems = MEMORY_USE*(2**17)/sum(map(len, diag)) # (number of MBytes x 2**20 bytes/MB x 8 bits/byte)/((64 bits/elem)* #samples)
...
try:
	...
	for i in range(len(output)):
		for j in range(len(output[i])):
			out_data[:, :, :] = output[i][j]
			nib.save(f, join(WORK_DIR, OUTPUT_DIR, OUTPUT_FILENAME + '_' + classes[i] + '_' + classes[i+j+1] + '.nii'))
except IOError:
	print
	...
	try:
		for i in range(len(output)):
			for j in range(len(output[i])):
				with open(join(WORK_DIR, OUTPUT_DIR, 'raw_' + OUTPUT_FILENAME + '_' + classes[i] + '_' + classes[i+j+1] + '.nii'), 'wb') as f:
