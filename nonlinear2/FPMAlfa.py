...
nib.save(niiFile(results.correction_parameters, affine), join(RESULTS_DIR, 'fpmalfa_pglm_cparams.nii'))
nib.save(niiFile(results.regression_parameters, affine), join(RESULTS_DIR, 'fpmalfa_pglm_rparams.nii'))
nib.save(niiFile(results.fitting_scores, affine), join(RESULTS_DIR, 'fpmalfa_pglm_fitscores.nii'))

with open(join(RESULTS_DIR, 'fpmalfa_pglm_userdefparams.txt'), 'wb') as f:
...
for fit_threshold in [0.99, 0.995, 0.999]:
	...
	nib.save(niiFile(z_scores, affine), join(RESULTS_DIR, 'fpmalfa_pglm_zscores_' + str(fit_threshold) + '.nii'))
	nib.save(niiFile(labels, affine), join(RESULTS_DIR, 'fpmalfa_pglm_labels_' + str(fit_threshold) + '.nii'))
